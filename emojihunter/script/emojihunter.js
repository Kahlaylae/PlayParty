...
const BOOST_DURATION = 3000; // ms (boost lasts 3 seconds - shows activated sprite)
let isCooling = false;
...
function drawDragon() {
    ...
    // Draw the Lottie canvas centered on the head position
...
function shootPellet() {
    ...
    // Boosted pellets are faster and deal more damage
    const speedMultiplier = isBoosting ? 1.5 : 1;
    ...
    pellet.size = isBoosting ? 10 : 8; // Bigger pellets during boost
    pellet.color = isBoosting 
        ? `hsl(${45 + Math.random() * 30}, 100%, 60%)` // Gold/orange during boost
        : `hsl(${Math.random() * 360}, 100%, 50%)`;
    pellet.timeAlive = 0;
    pellet.lifespan = 8.5;
    pellet.damage = isBoosting ? 2 : 1; // Double damage during boost
